...
class lstm_atten_v1(nn.Module):
    def __init__(self, embedding_dim=30, hidden_dim=128, num_layers=3):
        super(lstm_atten_v1, self).__init__()
        # Attention层
        self.attn = SelfAttention(n_layers=2, d_model=embedding_dim, n_heads=3, dropout=0.5)
    ...
    def forward(self, input):
        ...
        embedding = embedding.transpose(1, 2).contiguous()  # (batch_size, 1000, 60)
        embedding = embedding.transpose(0, 1).contiguous()  # (1000, batch_size, 60)

        # mask = get_padding_mask(embedding, 1000)
        # embedding = self.attn(embedding, mask)
        attention = self.attn(embedding)  # (1000, batch_size, 60)

        # LSTM层与全连接层
        encoding, _ = self.encoder(attention)  # (1000, batch_size, hidden_size)
        encoding = torch.cat((encoding[99], encoding[199], encoding[299], encoding[399], encoding[499], encoding[599], encoding[699], encoding[799], encoding[899], encoding[999]), -1)
        # encoding = encoding[-1]  # 取LSTM最后一个时刻的编码结果 (batch_size, hidden_size)
        decoding = self.decoder(encoding)  # (batch_size, 24)
...
class lstm_atten_v2(nn.Module):
    def __init__(self, embedding_dim=30, hidden_dim=15, atten_layers=2, lstm_layers=3):
        super(lstm_atten_v2, self).__init__()
        # Attention层
        self.attn = SelfAttention(n_layers=atten_layers, d_model=embedding_dim, n_heads=3, dropout=0.5)
    ...
    def forward(self, input):
        ...
        # LSTM层
        encoding, _ = self.encoder(attention)  # (1000, batch_size, 2*hidden_size)
