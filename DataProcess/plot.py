...
def generate_data(name):
    ...
    data = np.array(data)
    data_1000_sque = data[sque_mid - data_length // 2: sque_mid + data_length // 2]
    ...
    ax.plot(x, data_1000_fast[:, 0], linewidth=1.5)
    ax2 = plt.subplot(2, 1, 2)
    plt.xticks(fontname="Times New Roman")
    plt.yticks(my_x_ticks, fontname="Times New Roman")
    # plt.xticks(my_x_ticks)
    ax2.plot(x, data_1000_slow[:, 0], linewidth=1.5)
    legend.append(name[-1][:-4])
...
if __name__ == "__main__":
    ...
    for material in materials:
        if cnt < 3:
            ...
            cnt += 1
    plt.legend(legend)
    plt.savefig('p_total.png', bbox_inches='tight')
